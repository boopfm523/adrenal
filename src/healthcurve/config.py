...
class Settings(BaseSettings):
    ...
    #: Per-request text-model residency. Ollama defaults to five minutes; finite values
    #: are bounded to one hour, while Ollama's -1 sentinel keeps the selected text model
    #: resident indefinitely. Vision keeps Ollama's separate default.
    ollama_keep_alive_s: int = Field(default=300, ge=-1, le=3600)
    ...
    garmin_sync_hour_local: int = Field(default=12, ge=0, le=23)
