...
function response(url: string): Response {
  ...
  if (url.includes("/analytics/daily-patterns")) return jsonResponse({ date_from: "2026-08-01", date_to: "2026-08-02", timezone: "Europe/London", feature_version: "hc-daily-pattern-v1", safety_label: "Descriptive daily features do not establish causation or advise dosing.", definitions: { exposure: "Synthetic exposure definition.", wearable_coverage: "Synthetic observed coverage definition." }, exposure_model_versions: ["hc-exposure-v1"], longitudinal_summary: { total_days: 1, minimum_observed_days_for_trend: 7, coverage_definition: "Observed-day coverage is data availability only, not cortisol sufficiency.", multiple_comparison_caution: "Reviewing many metrics can surface chance patterns; correlation does not establish causation or diagnosis.", metrics: [{ key: "exposure_auc_reu_hours", label: "Theoretical exposure AUC", unit: "REU-hours", observed_days: 1, missing_days: 0, observed_day_percent: "100", minimum: "40", median: "40", maximum: "40", first_observed: "40", last_observed: "40", first_to_last_change: null, trend_eligible: false }], model_version_periods: [{ date_from: "2026-08-01", date_to: "2026-08-01", feature_version: "hc-daily-pattern-v1", exposure_model_version: "hc-exposure-v1" }] }, days: [{ date: "2026-08-01", timezone: "Europe/London", elapsed_hours: "24", feature_version: "hc-daily-pattern-v1", exposure_model_version: "hc-exposure-v1", dose_plan_version_ids: ["11111111-1111-4111-8111-111111111111"], source_revision_watermark_sha256: "a".repeat(64), supported_dose_count: 1, excluded_dose_count: 0, exposure_peak_reu: "10", exposure_peak_at: "2026-08-01T09:00:00Z", exposure_auc_reu_hours: "40", symptom_count: 1, symptom_severity_sample_count: 1, symptom_severity_missing_count: 0, average_symptom_severity: "7", symptom_timings: [{ symptom_event_id: "61111111-1111-4111-8111-111111111111", occurred_at: "2026-08-01T08:00:00Z", name: "Dizzy", severity: 7, previous_supported_dose_event_ids: ["11111111-1111-4111-8111-111111111111"], minutes_since_previous_supported_dose: "30", theoretical_exposure_reu: "8" }], wearables: [
...
describe("Analytics page", () => {
  it("renders definitions, timezone, missingness, and the no-causation caution", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Generate cited pattern draft" }));
    ...
    expect(within(seriesSummary).queryByText(/Garmin stress:/)).not.toBeInTheDocument();
