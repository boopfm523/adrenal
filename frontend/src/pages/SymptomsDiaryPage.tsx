...
export function SymptomsDiaryPage(): React.JSX.Element {
  ...
  return <Page title="Symptoms & diary" description="Subjective symptoms, diary notes, and life events are recorded facts—not diagnoses or causal claims.">
    ...
    <p className="privacy-note">Inclusive dates use {filters.timezone}. Sensitive text is hidden by default and appears only after applying the reveal control.</p>
    ...
    <section aria-labelledby="symptoms-heading"><h2 id="symptoms-heading">Symptoms</h2>
      ...
      {currentSymptoms.length === 0 ? null : <div className="table-scroll" tabIndex={0} role="region" aria-label="Symptom records table"><table><caption>Current symptom facts, latest experienced time first, with correction history.</caption><thead><tr><th scope="col">Experienced time</th><th scope="col">Symptom</th><th scope="col">Severity and area</th><th scope="col">Source and status</th><th scope="col">Notes and actions</th></tr></thead><tbody>{currentSymptoms.flatMap((item) => { const history = historyFor(item); const row = <tr key={item.id}><td className="timeline-time">{localTime(item.time.local_time)}<span>{item.time.timezone}</span></td><th scope="row">{item.name}</th><td>{item.severity === null ? "Not recorded" : `${item.severity.toString()}/10`}{item.body_area === null ? null : <span>{item.body_area}</span>}</td><td><span>{words(item.provenance.source_type)}</span><span>{words(item.provenance.confirmation_state)}</span><span>{item.provenance.is_correction ? `Corrected · ${item.provenance.correction_reason ?? "reason recorded"}` : "Original record"}</span></td><td>{item.notes ?? <span className="missing-value">None</span>}<button type="button" onClick={() => { setEditing(editing === item.id ? null : item.id); }}>{editing === item.id ? "Close correction form" : "Correct recorded symptom"}</button>{history.length === 0 ? null : <details className="revision-history"><summary>Revision history ({history.length})</summary>{history.map((prior) => <article key={prior.id}><p>{prior.name}{prior.severity === null ? "" : ` · severity ${prior.severity.toString()}/10`} · {localTime(prior.time.local_time)} · {prior.time.timezone}</p>{prior.body_area === null ? null : <p>Body area: {prior.body_area}</p>}{prior.notes === null ? null : <p>Notes: {prior.notes}</p>}</article>)}</details>}</td></tr>; return editing === item.id ? [row, <tr className="correction-table-row" key={`${item.id}-correction`}><td colSpan={5}><SymptomCorrectionForm symptom={item} close={() => { setEditing(null); }} /></td></tr>] : [row]; })}</tbody></table></div>}
    ...
    <section aria-labelledby="diary-heading"><h2 id="diary-heading">Diary</h2>
      ...
      {diary.data === undefined || diary.data.items.length === 0 ? null : <div className="table-scroll" tabIndex={0} role="region" aria-label="Diary records table"><table><caption>Recorded diary facts. Sensitive text appears only when explicitly revealed.</caption><thead><tr><th scope="col">Experienced time</th><th scope="col">Entry</th><th scope="col">Privacy and tags</th><th scope="col">Source and confirmation</th></tr></thead><tbody>{diary.data.items.map((item) => <tr key={item.id}><td className="timeline-time">{localTime(item.time.local_time)}<span>{item.time.timezone}</span></td><th scope="row">{item.text}</th><td>{item.is_sensitive ? "Sensitive" : "Not marked sensitive"}{item.tags === null || item.tags.length === 0 ? null : <span>{item.tags}</span>}</td><td><span>{words(item.provenance.source_type)}</span><span>{words(item.provenance.confirmation_state)}</span></td></tr>)}</tbody></table></div>}
