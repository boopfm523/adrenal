...
function EpisodeTable({ episodes, label, reviewEpisode = null }: { episodes: Episode[]; label: string; reviewEpisode?: string | null | undefined }): React.JSX.Element { return <div className="table-scroll" tabIndex={0} role="region" aria-label={`${label} table`}><table><caption>{label}, ordered by episode start time, latest first.</caption><thead><tr><th scope="col">Started</th><th scope="col">Trigger</th><th scope="col">Status and duration</th><th scope="col">Linked recorded facts</th><th scope="col">Recorded context and actions</th></tr></thead><tbody>{episodes.map((episode) => <EpisodeRow key={episode.id} episode={episode} review={episode.id === reviewEpisode} />)}</tbody></table></div>; }
