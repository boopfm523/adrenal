...
describe("Settings and privacy page", () => {
  ...
  it("discloses retention and sends reauthenticated privacy actions", async () => {
    ...
    expect(screen.getByText(/07:00 and 19:00 each day in your local timezone/)).toBeVisible();
    fireEvent.change(screen.getByLabelText("Garmin sync lookback (days)"), { target: { value: "7" } });
    fireEvent.click(screen.getByRole("button", { name: "Save sync window" }));
    await waitFor(() => { expect(requests.some((request) => request.url.endsWith("/integrations/garmin/settings") && request.method === "PATCH" && JSON.stringify(request.body) === JSON.stringify({ sync_lookback_days: 7 }))).toBe(true); });
    expect(await screen.findByText("Garmin sync window saved as 7 days.")).toBeVisible();
    ...
    const exportButton = screen.getByRole("button", { name: "Create private export" });
    ...
    expect(await screen.findByText("Private export queued. Progress appears below.")).toBeVisible();
  ...
  it("refreshes Garmin status until a manually queued sync completes", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation((input, init) => {
      ...
      if (url.endsWith("/integrations/garmin/status")) {
        ...
        return Promise.resolve(new Response(JSON.stringify({
          configured: true,
          state: "connected",
          automatic_sync_hour_local: 7,
          automatic_sync_interval_hours: 12,
    ...
    expect(screen.getByText(/07:00 and 19:00/)).toBeVisible();
    fireEvent.click(screen.getByRole("button", { name: "Sync Garmin now" }));
