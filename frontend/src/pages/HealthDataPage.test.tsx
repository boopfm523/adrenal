...
const weight = { id: "22222222-2222-4222-8222-222222222222", category: "fact", value: "180.0000", unit: "lb", normalized_kg: "81.6466", time, provenance, notes: null };
...
describe("Health data page", () => {
  ...
  it("renders equivalent trend tables, explicit units, missingness, and correction provenance", async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation((input, init) => {
      ...
      return Promise.resolve(json(url.includes("blood-pressure") ? [corrected, pressure] : [weight]));
    });
    renderPage();
    ...
    expect(screen.getByRole("columnheader", { name: "Normalized weight (kg)" })).toBeInTheDocument();
    expect(screen.getByText("Corrected · Synthetic correction")).toBeVisible();
    expect(screen.getAllByText(/Source: telegram · confirmed from draft/)).toHaveLength(3);
