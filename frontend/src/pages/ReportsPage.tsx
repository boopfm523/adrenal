...
export function ReportsPage(): React.JSX.Element {
  ...
  const history = useQuery({ queryKey: ["reports", historyPage], queryFn: () => getReports(historyPage) });
  const preview = useQuery({ queryKey: ["report", selectedId], queryFn: () => getReport(selectedId ?? ""), enabled: selectedId !== null });
  const generation = useMutation({ mutationFn: createReport, onSuccess: async (created) => { setSelectedId(created.id); setHistoryPage(1); await queryClient.invalidateQueries({ queryKey: ["reports"] }); } });
