...
The pinned `garminconnect==0.3.9` implementation exposes four read-only, single-date
methods. A structure-only probe from the isolated Garmin worker on 2026-08-11 verified
...
1. Expand the narrow adapter allow-list to exactly `get_heart_rates`,
   `get_stress_data`, `get_respiration_data`, and `get_hrv_data`, in addition to the
   three reads approved by ADR-0012. No general Garmin client escapes the adapter and
   no mutating method is permitted.
...
8. Retain every security, isolation, disconnect/deletion, export, correction, fallback,
